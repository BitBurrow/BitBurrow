...
class nodes:
    ...
    async def node_1(self, outbound_port):
        ...
        for rep in range(1, last_rep + 1):
            pws_log.debug(f"B53991 connection test {rep} of {last_rep}")
            try:
                ...
                pws_log.debug(f"B60192 exception {e}")
                peer_connection = None
            else:
                ...
                pws_log.debug(f"B11890     {round(time_lap1 * 1000)} ms, {len(expected)} chars")
                if rep < last_rep:
                ...
                pws_log.debug(f"B80929     TCP disconnect took {round(time_lap2 * 1000000)} µs")
        pws_log.debug(f"B97965 node_{node_num}_complete")
    ...
    async def node_4(self, listen_port, ready):
        ...
        try:
            ...
            pws_log.debug("B38045 node_4 complete")
...
class ActiveTcpConnection(asyncio.Protocol):
    ...
    def connection_made(self, transport: asyncio.BaseTransport):
        ...
        if len(self._connections) > 1:
            pws_log.debug(f"B58943 node {self._node_num} too many connections")
            ...
            for line in lines[0:-1]:  # process all complete lines
                ...
                match cmd:
                    ...
                    case '':
                        pass
                    case _:
                        pws_log.debug(f"B88929 node_{self._node_num} unknown cmd: {repr(cmd)}\n")
    ...
    def write(self, data: str):
        if not isinstance(self.transport, asyncio.WriteTransport):
            pws_log.debug(f"B17634 node {self._node_num} cannot write because connection is closed")
            return
        try:
            self.transport.write(data.encode())
        except Exception as e:
            pws_log.debug(f"B45119 node {self._node_num} exception {e}")
