...
messages: Dict[str, persistent_websocket.PersistentWebsocket] = dict()  # one entry for each cid


@router.websocket('/rpc{rpc_ver}/{auth_account}/{cid}')
async def rpc(websocket: WebSocket, rpc_ver: str, auth_account: str, cid: str):
    ...
    :param cid: connection ID; random string generated by app to uniquely identify this WebSocket
    """
    account = db.Account.validate_login_key(auth_account, allowed_kinds=db.admin_manager_or_coupon)
    if cid not in messages:
        messages[cid] = persistent_websocket.PersistentWebsocket(cid)
        persistent_websocket.logger.setLevel(logging.DEBUG)
    try:
        await websocket.accept()
        async for m in messages[cid].connected(websocket):
            logger.info(f"------------------------------- {cid} incoming: {m.decode('utf-8')}")
    except persistent_websocket.PWUnrecoverableError:
        del messages[cid]  # data is no longer usable
